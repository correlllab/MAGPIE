...
from PB_BlocksWorld import PB_BlocksWorld, rand_table_pose, _trgtRed, _trgtGrn
...
from env_config import ( _BLOCK_SCALE, _N_CLASSES, _CONFUSE_PROB, _NULL_NAME, _NULL_THRESH, 
                         _BLOCK_NAMES, _VERBOSE, _N_XTRA_SPOTS, _ACCEPT_POSN_ERR, _MIN_SEP, 
...
class BaselineTAMP:
    ...
    def get_placement_pose_stream( self ):
        ...
        def stream_func( *args ):
            ...
            while not placed:
                ...
                if not collide:
                    placed = True
            rtnObj = ObjPose( testPose )
            print( f"\nPLACEMENT POSE SUCCESS: {rtnObj}\n" )
            yield (rtnObj,)
    ...
    def set_goal( self ):
        ...
        self.goal = ( 'and',
            ...
            ('GraspObj' , 'redBlock', _trgtRed  ), 
            ('GraspObj', 'grnBlock' , _trgtGrn  ), 
            ('HandEmpty',),
        )
    ...
    def phase_2_Conditions( self ):
        ...
        if not self.check_goal_objects( self.goal, self.symbols ):
            ...
            for sym in self.symbols:
                ...
                #     self.facts.append( ('GraspObj', sym.label, blockPose,) )
                # if p_goalDex or (not (p_factDex or p_goalDex)):
            ...
            testFunc = self.get_free_placement_test()
            for _ in range( _N_XTRA_SPOTS ):
                nuPose = ObjPose( pb_posn_ornt_to_row_vec( *rand_table_pose() ) )
                self.facts.append( ('Waypoint', nuPose,) )
                if testFunc( nuPose ):
                    self.facts.append( ('Free', nuPose,) )
